...
const isPlaceholderUrl =
  hasUrl &&
  (String(supabaseUrl).includes('seu-projeto.supabase.co') ||
    String(supabaseUrl).includes('your-project.supabase.co'));
const isPlaceholderKey =
  hasKey &&
  (String(supabaseServiceKey) === 'sua_service_role_key_aqui' ||
    String(supabaseServiceKey).startsWith('sua_') ||
    String(supabaseServiceKey).length < 40);
...
if (!hasUrl || !hasKey) {
  ...
  console.warn('⚠️  Supabase com credenciais de exemplo — substitua no backend/.env pelos valores reais do painel Supabase.');
} else {
...
/**
 * Busca compra por id_pagamento (idempotência de pagamentos)
 */
export const getPurchaseByPaymentId = async (paymentId) => {
  if (!supabaseAdmin || !paymentId) {
    return null;
  }
  ...
  return mapPurchaseToEnglish(data);
};
